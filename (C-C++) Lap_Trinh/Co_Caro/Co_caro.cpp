...
int main(){
	SetConsoleTitle("Tro Choi: Co Caro - Tac gia: LE QUANG THO");
	khung();
	khungtiso();
	gotoxy(x,y);
	play();
...
void khung() {
	...
	txtColor(14);
	gotoxy(10,21);
	cout << "Le Quang Tho tham khao tu Duong Chi Dung. ";
	gotoxy(10,22);
	cout << "Facebook: facebook.com/ ";
	huongdan();
}
...
void play() {
	...
	if(thang())
	{
		...
		cout << " ";
		gotoxy(10,1);
		cout << "Nguoi choi " << thang() << " da thang.\1";
		if(thang() == 1) p1++; 
		else p2++;
		for(int k = 0; k < 4; k++)
		{
